import { Compute, Produce } from '../types';
import Err from './Err';
import Ok from './Ok';
...
export function match<T, E, U, V>(result: Result<T, E>, okFn: Compute<T, U>, errFn: Compute<E, V>): U | V {
    return result.isOk() ? okFn(result.unwrap()) : errFn(result.unwrapErr());
}
